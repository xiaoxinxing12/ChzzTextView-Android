...
import org.chzz.textview.util.DisplayUtils;
...
public class LineText extends HText {
    ...
    @Override protected void initVariables() {
        ...
        xLineWidth = DisplayUtils.dp2px(mCHZZTextView.getContext(), 1.5f);
        padding = DisplayUtils.dp2px(mCHZZTextView.getContext(), 15);

        linePaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        linePaint.setColor(mCHZZTextView.getCurrentTextColor());
    ...
    @Override protected void animatePrepare(CharSequence text) {
        ...
        xLineLength = mCHZZTextView.getWidth();
        yLineLength = mCHZZTextView.getHeight();
    }
    ...
    @Override protected void drawFrame(Canvas canvas) {
        ...
        // 计算横向 和 纵向 线条的最终宽度
        xLineLength = (int) (mCHZZTextView.getWidth() - (mCHZZTextView.getWidth() - distWidth + gap) * percent);
        yLineLength = (int) (mCHZZTextView.getHeight() - (mCHZZTextView.getHeight() - distHeight + gap) * percent);

        p1.x = (mCHZZTextView.getWidth() / 2 + distWidth / 2 - gap + xLineWidth/2) * percent;
        p1.y = (mCHZZTextView.getHeight() - distHeight) / 2;
        canvas.drawLine(p1.x - xLineLength, p1.y, p1.x, p1.y, linePaint);

        p2.x = (mCHZZTextView.getWidth() / 2 + distWidth / 2);
        p2.y = (mCHZZTextView.getHeight() / 2 + distHeight / 2 - gap+xLineWidth/2) * percent;
        canvas.drawLine(p2.x, p2.y - yLineLength, p2.x, p2.y, linePaint);

        p3.x = mCHZZTextView.getWidth() - (mCHZZTextView.getWidth() / 2 + distWidth / 2 - gap + xLineWidth/2) * percent;
        p3.y = (mCHZZTextView.getHeight() + distHeight) / 2;
        canvas.drawLine(p3.x + xLineLength, p3.y, p3.x, p3.y, linePaint);

        p4.x = (mCHZZTextView.getWidth() / 2 - distWidth / 2);
        p4.y = mCHZZTextView.getHeight() - (mCHZZTextView.getHeight() / 2 + distHeight / 2 + gap + xLineWidth/2) * percent;
        ...
        p5.x = (mCHZZTextView.getWidth() / 2 + distWidth / 2);
        p5.y = (mCHZZTextView.getHeight() - distHeight) / 2;
        canvas.drawLine(p5.x - xLineShort, p5.y, p5.x, p5.y, linePaint);

        p6.x = (mCHZZTextView.getWidth() / 2 + distWidth / 2);
        p6.y = (mCHZZTextView.getHeight() / 2 + distHeight / 2);
        canvas.drawLine(p6.x, p6.y - yLineShort, p6.x, p6.y, linePaint);

        p7.x = mCHZZTextView.getWidth() - (mCHZZTextView.getWidth() / 2 + distWidth / 2 - gap);
        p7.y = (mCHZZTextView.getHeight() + distHeight) / 2;
        canvas.drawLine(p7.x + xLineShort, p7.y, p7.x, p7.y, linePaint);

        p8.x = (mCHZZTextView.getWidth() / 2 - distWidth / 2);
        p8.y = mCHZZTextView.getHeight() - (mCHZZTextView.getHeight() / 2 + distHeight / 2 - gap);
